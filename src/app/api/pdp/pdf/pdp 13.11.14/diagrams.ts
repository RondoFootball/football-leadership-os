...
export function svgTimeline8Weeks(
  ...
  const xForWeek = (wk: number) =>
    lineX1 +
    ((Math.max(1, Math.min(weeks, wk)) - 1) / (weeks - 1)) * (lineX2 - lineX1);

  const nodes = (checkpoints || [])
    .slice(0, 10)
    .map((c) => {
      const x = xForWeek(c.week);
      const label = (c.label || "").slice(0, 16);
      return `
        <g>
  ...
  const weekTicks = Array.from({ length: weeks })
    .map((_, i) => {
      const wk = i + 1;
      const x = xForWeek(wk);
  ...
  return `
  <svg viewBox="0 0 ${w} ${h}" width="100%" height="${h}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="8-week timeline">
    <rect x="0" y="0" width="${w}" height="${h}" rx="14" fill="var(--surface-1)"/>
    <line x1="${lineX1}" y1="${y}" x2="${lineX2}" y2="${y}" stroke="rgba(0,0,0,.18)" stroke-width="2" stroke-linecap="round"/>
    ${weekTicks}
    ${nodes}
  </svg>`;
}
...
export function svgBehaviourLadder(
  ...
  const nodes = labels
    .map((t, idx) => {
      ...
      <g>
        ...
        <circle cx="${xLine}" cy="${y}" r="14" fill="${accent}" opacity="${
        active ? 0.12 : 0.06
      }"/>
        <text x="${xLine + 26}" y="${y + 4}" font-size="12" fill="rgba(0,0,0,.78)">${esc(
          t
        )}</text>
      </g>`;
    })
  ...
  return `
  <svg viewBox="0 0 ${w} ${h}" width="100%" height="${h}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Behaviour ladder">
...
export function svgStoryboard3(
  ...
  const frame = (x: number, title: string, body: string) => `
    <g>
      ...
      <text x="${x + 14}" y="${fy + 26}" font-size="10" fill="rgba(0,0,0,.55)" letter-spacing="2">${esc(
        title
      )}</text>
      <text x="${x + 14}" y="${fy + 52}" font-size="12" fill="rgba(0,0,0,.78)">${esc(
        body
      )}</text>
    </g>`;

  return `
  <svg viewBox="0 0 ${w} ${h}" width="100%" height="${h}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Match moment storyboard">
    <rect x="0" y="0" width="${w}" height="${h}" rx="14" fill="var(--paper)" stroke="var(--hairline)"/>
...
export function svgLoadIndicator(value01: number, opts: SvgOpts = {}) {
  ...
  return `
  <svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Load indicator">
